...
module.exports = class CyberTruck{
    ...
    static async insert({ title, descript, color }) {
      const { rows } = await pool.query(
        'INSERT INTO cyber_truck (title, descript, color,) VALUES ($1, $2, $3) RETURNING *',
    ...
    static async find() {
      const { rows } = await pool.query('SELECT * FROM cyber_truck');
    ...
    static async findById(id) {
      const { rows } = await pool.query(
        'SELECT * FROM cyber_truck WHERE id=$1',
        [id]
      );
    ...
    static async delete(id) {
      const { rows } = await pool.query(
        'DELETE FROM cyber_truck WHERE id=$1 RETURNING *',
        [id]
      );
